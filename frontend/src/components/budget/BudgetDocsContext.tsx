...
import { BudgetGuideContent } from './BudgetGuideContent'
import { BUDGET_DOC_SECTION_IDS, budgetDocsScrollStorageKey, type BudgetDocsSection } from './budgetDocAnchors'

const STORAGE = budgetDocsScrollStorageKey()

type PendingOpen = { mode: 'restore' } | { mode: 'section'; section: BudgetDocsSection }

export type BudgetDocsContextValue = {
  /** Open guide modal. Omit `section` to restore saved scroll position. */
  openDocs: (section?: BudgetDocsSection) => void
}

const BudgetDocsContext = createContext<BudgetDocsContextValue | null>(null)

export function useBudgetDocs(): BudgetDocsContextValue {
  const v = useContext(BudgetDocsContext)
  if (!v) throw new Error('useBudgetDocs requires BudgetDocsProvider')
  return v
}
...
export function BudgetDocsSectionLink(props: {
  section: BudgetDocsSection
  className?: string
  /** Override default link copy. */
  label?: string
}) {
...
function Modal(props: {
  ...
  useLayoutEffect(() => {
    ...
    requestAnimationFrame(() => {
      if (props.pendingOpen.mode === 'section') {
        const domId = BUDGET_DOC_SECTION_IDS[props.pendingOpen.section]
  ...
  return (
    <div
      ...
      aria-modal="true"
      aria-labelledby="budget-docs-modal-title"
      role="dialog"
    >
      <div
        ref={backdropRef}
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <div>
            <h2 id="budget-docs-modal-title" className="text-lg font-semibold text-slate-800">
              Budget allocation guide
            </h2>
            <p className="mt-1 max-w-xl text-[11px] leading-snug text-slate-500">
              Scroll position is remembered when you close and reopen from the Docs button. Contextual links jump straight
              to a topic.
            </p>
          </div>
          <button
...
export function BudgetDocsProvider(props: { children: ReactNode }) {
  ...
  const openDocs = useCallback((section?: BudgetDocsSection) => {
