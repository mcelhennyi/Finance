...
| **`/develop-frontier`** | Compare pinned/remote skeleton hashes before every wave and sync only when changed; then run one subagent per parallel-capable ticket (**TEST→DEV→VAL** per child worktree under **`.worktrees/FR-NNNN-<slug>/`**), merge to **`feat/…`**, and use **`finish-feature`** only after the **§2d** gate. |
...
| **`/feature-status`** | Give a concise plain-English feature update with a BLUF, tickets-left and waves-left fractions, and a table of **Done**, **Upcoming**, **Blocked**, and **Skipped** work with rough complexity signals — **`.cursor/skills/feature-status/SKILL.md`**. |
