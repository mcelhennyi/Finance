...
- Sections or paths that need named domain judgment in the default-branch PR use **`EXPERT-REVIEW:`** with one or more **`EXPERT:<slug>`** tags. Resolve them through **`docs/design/EXPERTS.md`** plus **`EXPERTS.project.md`** when present; full protocol: **`docs/design/expert-review.md`**.
...
**`Deps:`** list other tickets by **full id** or `none` in canonical
**`tickets.md`** so dependency tooling stays stable. In human-facing DAG tables,
show each dependency as **title + stable id** and render `none` as a short phrase
such as “Nothing — can start immediately.”
...
**Mermaid ticket nodes:** Stable Mermaid node ids and dependency edges stay
machine-readable, but every visible node label leads with the canonical ticket
title and keeps the stable ticket id second. Phase triads translate the phase
for readers — **Write the checks** (`TEST`), **Build the change** (`DEV`), and
**Verify it works** (`VAL`) — while retaining the phase code in parentheses.
For **`T-FR-NNNN-xx`**, triad node ids remain **`TFR` + `NNNN` + `_` + `xx` +
`_` + `TEST|DEV|VAL`**. When a ticket is fully complete, add the corresponding
`class … triadDone` line in **`docs/design/tickets-initial.md`** (see that file).

**DAG status prose:** Immediately below every ticket Mermaid block, keep a
plain-English **Where things stand** note of no more than three sentences: one
sentence for overall project progress and one for the feature or combined graph
shown. Run **`python3 scripts/refresh_ticket_dags.py --root .`** whenever tickets,
dependencies, or tracker phases change; the command derives titles, dependency
annotations, lifecycle colors, and status prose without changing stable ids or
edges.
...
8. **Expert review gates** — Put a blockquote labeled **`EXPERT-REVIEW:`** followed by **`EXPERT:<slug>`** beside the authoritative section when named domain review is required. Multiple tags all apply. Carry the tags without blocking ticket work; during default-branch PR preparation, map paths plus semantic effects, request targeted reviewers, and record scoped **`EXPERT-APPROVAL`** evidence. Missing mappings or approvals block only the final default-branch merge; see **`docs/design/expert-review.md`** and the mirrored **`expert-review`** rules.
