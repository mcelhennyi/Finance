...
The responsive Budget width follow-up makes allocation and account tables fit available screen width instead of depending on horizontal scrolling. Allocation account linkage is treated as graph/modal detail rather than a primary table column; obvious source/sink semantics use row color and short badges; lower-priority fields such as notes, masks, binary active state, and repetitive account details collapse before core names, amounts, timing, and actions. Dense edit flows use wide/full-screen modals so every field remains editable without crowding the table. Mock: [`mockups/fr-0006-responsive-budget-width.html`](mockups/fr-0006-responsive-budget-width.html).
